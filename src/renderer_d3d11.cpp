...
//			invalidateCache();
	...
	void* TextureD3D11::create(const Memory* _mem, uint64_t _flags, uint8_t _skip)
	{
		...
		if (bimg::imageParse(imageContainer, _mem->data, _mem->size) )
		{
			...
			for (uint16_t side = 0; side < numSides; ++side)
			{
              ...
              uint8_t* temp = NULL;
							...
							case TextureFormat::RGB5A1:
								...
								bimg::imageConvert(temp, 16, bx::packBgr5a1, mip.m_data, bx::unpackRgb5a1, size);
								srd[kk].pSysMem = temp;
								break;

							case TextureFormat::RGBA4:
								temp = (uint8_t*)BX_ALLOC(g_allocator, srcpitch*mip.m_height);
								bimg::imageConvert(temp, 16, bx::packBgra4, mip.m_data, bx::unpackRgba4, size);
								srd[kk].pSysMem = temp;
								break;
							}
			...
			if ((convert
			|| m_textureFormat == TextureFormat::RGB5A1
			|| m_textureFormat == TextureFormat::RGBA4)
			&&  0 != kk)
			{
				kk = 0;
				for (uint16_t side = 0; side < numSides; ++side)
